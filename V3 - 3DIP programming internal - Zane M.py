    ...
    while True:       
        if muscle_name in exercises:
            ...
            if len(exercise_list) == 1:
                return f"For {muscle_name}, a recommended exercise is: {exercise_list[0]}. This will specifically hit the {others_sub_muscle} and the {last_sub_muscle}."
            else:
                other_exercises = ", ".join(exercise_list[:-1])
                last_exercise = exercise_list[-1]
                return f"For {muscle_name}, you can do: {other_exercises}, and {last_exercise}. These will specifically hit the {others_sub_muscle} and the {last_sub_muscle}."
        else:
